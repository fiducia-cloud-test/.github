...
write_contract() {
  ...
  rm -rf "$work/tests/__pycache__"

  touch "$work/.gitignore"
  grep -qxF '__pycache__/' "$work/.gitignore" || printf '__pycache__/\n' >> "$work/.gitignore"
  grep -qxF '*.py[cod]' "$work/.gitignore" || printf '*.py[cod]\n' >> "$work/.gitignore"
...
class EngineFocusE2E(unittest.TestCase):
    def test_repository_engine_and_focus_identity(self):
        ...
        self.assertIn(
            SCENARIO["integrationMode"],
            {"local-container", "local-process", "advisory", "protected-environment"},
        )
    ...
    def test_policy_fails_closed_without_pull_request_credentials(self):
        ...
        self.assertEqual(
            policy["liveEnvironmentRequiredForCertification"],
            SCENARIO["integrationMode"] == "protected-environment",
        )
  ...
  contract:
    ...
    steps:
      ...
      - name: Validate GitHub Actions
        ...
        with:
          args: .github/workflows/engine-focus.yml
      ...
      - name: Reject generated Python bytecode
        run: test -z "$(find tests -type f -name '*.pyc' -print -quit)"
EOF
}
...
publish_repository() {
  ...
  local branch="agent/engine-focus-e2e-v2"
  local work="${RUNNER_TEMP:-/tmp}/$repository"
  local existed=false
  local title body pr_number pr_url head_sha
  ...
  [[ -f "$work/tests/test_engine_focus.py" ]] && existed=true
  write_contract "$work" "$full"

  (
    cd "$work"
    PYTHONDONTWRITEBYTECODE=1 python3 -m unittest -v tests/test_contract.py tests/test_engine_focus.py
    test -z "$(find tests -type f -name '*.pyc' -print -quit)"
    git add -A
    if ! git diff --cached --quiet; then
      if [[ "$existed" == true ]]; then
        git commit -m "test: harden engine focus E2E and remove bytecode artifacts"
      else
        git commit -m "test: add engine and focus E2E contract"
      fi
    fi
    if (( $(git rev-list --count origin/main..HEAD) == 0 )); then
      emit "$organization" "$repository" pull_request unchanged "main already contains the hardened engine/focus contract"
      exit 0
    fi
  ...
  if [[ "$existed" == true ]]; then
    title="test: harden engine focus E2E and remove bytecode artifacts"
    body="Removes generated Python bytecode, adds ignore and no-bytecode gates, preserves protected-environment certification for cloud warehouses, and reruns all canonical and engine-specific journeys."
  else
    title="test: add engine and focus E2E contract"
    body="Adds three executable checks for repository and engine identity, required invariant uniqueness, and fail-closed credential-free migration policy. Protected cloud environments remain live-certified without exposing credentials to pull requests."
  fi
  ...
  if [[ -z "$pr_number" ]]; then
    ...
    gh pr edit "$pr_number" -R "$full" --title "$title" --body "$body" >/dev/null
...
audit_e2e_repository() {
  ...
  if gh repo view "$full" >/dev/null 2>&1; then
    ...
    emit "$organization" "$repository" repository missing "repository is absent"
    return 0
  fi

  pr_url="$(gh pr list -R "$full" --state merged --limit 20 --json title,url \
    --jq '[.[] | select(.title == "test: add three mandatory process-level E2E journeys")][0].url // empty')"
  if [[ -n "$pr_url" ]]; then
    emit "$organization" "$repository" pull_request merged "$pr_url"
    add_project_item "$organization" "$pr_url"
  else
    emit "$organization" "$repository" pull_request missing "mandatory certification PR not found"
  fi
}
...
audit_e2e_repository cliptown/cliptown-e2e
audit_e2e_repository memebank/memebank-e2e
audit_e2e_repository meta-agents-demo/metacog-e2e
audit_e2e_repository unreal-unity-poc/unreal-unity-poc-e2e
...
jq -s '{generatedAt:(now|todateiso8601),events:.,summary:{total:length,verifiedRepositories:([.[]|select(.action=="repository" and .status=="verified")]|length),createdPullRequests:([.[]|select(.action=="pull_request" and .status=="created")]|length),mergedPullRequests:([.[]|select(.action=="merge" and .status=="merged")]|length),blockedMerges:([.[]|select(.action=="merge" and .status=="blocked")]|length),pendingMerges:([.[]|select(.action=="merge" and .status=="pending")]|length),createdProjects:([.[]|select(.action=="project" and .status=="created")]|length),reusedProjects:([.[]|select(.action=="project" and .status=="reused")]|length),blockedProjects:([.[]|select(.action=="project" and .status=="blocked")]|length)}}' \
  "$events" > "$out/summary.json"
{
  echo '# Organization E2E and CI finalization'
  echo
